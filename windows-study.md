...
即使 `netstat -ano` 没有显示，也可能是端口处于 `TIME_WAIT` 状态，导致无法正常使用。
...
| 命令                       | 说明                                                         |
| -------------------------- | ------------------------------------------------------------ |
| xcopy wg h:\temp1\wg /e    | 复制wg文件夹下的所有东西到 目标文件夹下 /e: 表示复制子目录包括空目录 /s:不包括空目录 , 实际应用还是用 robocopy, 如果<br />目标文件夹下没有wg这个文件夹, 会询问你 wg 是文件还是文件夹 |
| robocopy wg h:\temp1\wg /e | 同上, 更强大, 用于大量数据的复制, 具有错误恢复, 详细日志等高级功能 |
|                            |                                                              |
|                            |                                                              |
|                            |                                                              |
|                            |                                                              |
